...
async function getUserStreak(discordId) {
...
async function updateUserStreak(discordId, streak, lastClaim) {
    if (!db) throw new Error('Database not initialized');
...
export async function execute(interaction) {
    try {
        ...
        const hasMultiplierRole = interaction.member?.roles?.cache?.has(MULTIPLIER_ROLE_ID) || false;
...
export async function getAllStreaks() {
    ...
    try {
        return await db.all('SELECT * FROM user_streaks ORDER BY streak DESC');
    } catch (error) {
